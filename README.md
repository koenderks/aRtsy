...
The `paint_strokes` algorithm is my first self-concocted algorithm for drawing on a grid-based canvas. The algorithm is based on the premisse that each point has a large chance to take over the color of an adjacent colored point, but also has a slight change of generating a new color. This causes these brush-like strokes on the canvas. 
