...
paint_diamonds <- function(colors, background = '#fafafa', col.line = 'black', 
  ...
  palette <- NULL
  canvas <- data.frame(x = numeric(), y = numeric(), type = numeric())
  ...
  artwork <- ggplot2::ggplot(data = canvas, mapping = ggplot2::aes(x = x, y = y, group = type)) +
    ...
    ggplot2::geom_curve(data = data.frame(x = 0, y = sample(0:height/2, size = 1), xend = width, yend = sample((height/2):height, size = 1), type = 999 ),
    ...
    ggplot2::geom_polygon(fill = canvas$col, color = background, alpha = alpha, size = size) +
