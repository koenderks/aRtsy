...
if (type == 1) {
  ...
  artwork <- aRtsy::canvas_planet(colors = list(randomcoloR::randomColor(3)), 
                                  iterations = 30, 
                                  starprob = runif(1, 0.001, 0.1))
  
} else if (type == 7) {
  
  artwork <- aRtsy::canvas_circlemap(colors = randomcoloR::randomColor(3, luminosity = "dark"),
                                     x_min = runif(1, -4, 0),
                                     x_max = runif(1, 1, 14),
                                     y_min = 0,
                                     y_max = 1,
  ...
  artwork <- aRtsy::canvas_ribbons(colors = randomcoloR::randomColor(sample(3:6, size = 1), luminosity = "dark"),
                                   background = randomcoloR::randomColor(1, luminosity = "light"))
  
} else if (type == 9) {
  
  artwork <- aRtsy::canvas_polylines(colors = randomcoloR::randomColor(sample(3:6, size = 1), luminosity = "dark"),
                                     background = sample(c("#fafafa", "black", randomcoloR::randomColor(1, luminosity = "light")), size = 1))
  
} else if (type == 10) {
  
  artwork <- aRtsy::canvas_diamonds(colors = randomcoloR::randomColor(sample(4:8, size = 1)), 
                                    background = sample(c("#fafafa", "black", randomcoloR::randomColor(1, luminosity = "light")), size = 1),
